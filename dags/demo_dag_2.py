...
def generate_random_numbers(**kwargs):
    ...
    # Push numbers to XCom (component to send output of one function as input of another) for other tasks to consume
    kwargs['ti'].xcom_push(key='random_numbers', value=numbers) # it is saving it for future use if somone pulls downstream
    # ti is task instance , it get the 
    return numbers
